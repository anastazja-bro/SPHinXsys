...
namespace SPH
{
		...
		computeErrorAndParameters(size_t index_i, Real dt)
	{
		...
		for (size_t k = 0; k < this->contact_configuration_.size(); ++k)
		{
			...
			for (size_t n = 0; n != contact_neighborhood.current_size_; ++n)
			{
				...
				/* The temperature correction and volumetric source term is not used now.*/
				// The temperature correction on boundary particle.
				//VariableType variable_derivative_Neumann = (variable_i - variable_k[index_j]);
				//VariableType variable_derivative_Dirichlet = (1 + normal_distance_k[index_j] / 
				//	(this->normal_distance_[index_i] + TinyReal)) * (variable_i - variable_k[index_j]);
				//The volumetric source term for Neumann BC.
				//Real boundary_heat_flux = heat_flux_k[index_j] * Vol_k[index_j] * 
				//	dot((this->normal_vector_[index_i] - normal_vector_k[index_j]), (dW_ij_ * e_ij_)) * dt;
				
				/*The boundary particle with temperature value will be calculated the error. */
		...
		interaction(size_t index_i, Real dt)
	{
		...
		{
			this->residual_T_global_[index_i] = 0.0;
		}
	}
	//=================================================================================================//
	template <class BaseParticlesType, class BaseMaterialType, typename VariableType, int NUM_SPECIES>
